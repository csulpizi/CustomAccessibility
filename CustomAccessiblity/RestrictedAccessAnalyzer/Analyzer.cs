...
public partial class RestrictedAccessAnalyzer : DiagnosticAnalyzer
{
    ...
    protected void ValidateUsingStatic(SyntaxNodeAnalysisContext ctx)
    {
    ...
    protected void ValidateTypeDeclaration(SyntaxNodeAnalysisContext ctx)
    {
    ...
    )
    {
        ...
        foreach (var typeReference in descendents)
        {
            ...
            if (validation is ValidationResult.NoError)
                ReportNoAccessIssueForSymbol(ctx, symbol, typeReference);
            else if (validation is ValidationResult.ImplicitlyRestrictedAccess)
                ReportCACC001(ctx, symbol, typeReference.GetLocation());
            else
                ReportCACC000(ctx, symbol, typeReference.GetLocation());
        }
    }   
}
